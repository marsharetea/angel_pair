<?php
    ...
    function pressLike() {
        ...
        if (!checkLike($articleID, $userID)) {
            $statement = mysqli_prepare($con, "INSERT INTO presslike_complain (articleid, userid) VALUES (?, ?)");
    ...
    function recoverLike() {
        global $con, $articleID, $userID;
        if (checkLike($articleID, $userID)) {
            $statement = mysqli_prepare($con, "DELETE FROM presslike_complain WHERE articleid = ? AND userid = ?");
            mysqli_stmt_bind_param($statement, "ii", $articleID, $userID);
            mysqli_stmt_execute($statement);
